        ...
        for (int i = 0; i < n; i++) {
            if (j < m && S.charAt(i) == W.charAt(j)) {
                ...
                if (i > 1 && S.charAt(i) == S.charAt(i - 1) && S.charAt(i - 1) == S.charAt(i - 2)) {
                    ...
                    if (0 < i && i < n - 1 && S.charAt(i - 1) == S.charAt(i) && S.charAt(i) == S.charAt(i + 1)) {
                        continue;
                    }  else {
                        return false;
                    }
    ...
    public String customSortString(String S, String T) {
        ...
        for (int i=0; i<S.length(); i++) {
            if (map.containsKey(S.charAt(i))) {
                ...
                for (int j=1; j<=count; j++) sb.append(S.charAt(i));
                map.remove(S.charAt(i));
            }
        }
        for (Map.Entry<Character, Integer> entry : map.entrySet()) {
            for (int j=1; j<=entry.getValue(); j++) sb.append(entry.getKey());
        }
        return sb.toString();
    }

    public String customSortString_1(String S, String T) {
