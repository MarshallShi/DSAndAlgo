...
public class MinimaxAlgoExe {
    ...
    public int stoneGameII_2(int[] piles) {
        int sum = 0;
        for (int v : piles) sum += v;
        Integer[][] dp = new Integer[piles.length][piles.length];
        int diff = dfs(piles, 0, 1, dp);
        return (sum + diff)/2;
    }

    private int dfs(int[] A, int pos, int m, Integer[][] dp) {
        if (pos >= A.length) {
            return 0;
        }
        if (dp[pos][m] != null) {
        ...
        int temp = 0, localOptimal = Integer.MIN_VALUE;
        for (int i=pos; i<pos+m*2 && i<A.length; i++) {
            temp = temp + A[i];
            int nextM = Math.max(i - pos + 1, m);
            localOptimal = Math.max(localOptimal, temp - dfs(A, i+1, nextM, dp));
        }
        dp[pos][m] = localOptimal;
        return localOptimal;
    }
