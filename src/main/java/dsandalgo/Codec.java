...
//        System.out.println(node);
    ...
    public String serialize(TreeNode root) {
        ...
        String[] result = new String[(int)Math.round(Math.pow(2,height) - 1)];
        serializeHelper(root, result, 0, result.length - 1);
    ...
    private void serializeHelper(TreeNode root, String[] result, int start, int end) {
        if (root == null) {
            return;
        }
        ...
        result[mid] = root.val + "";
        if (start < end) {
            serializeHelper(root.left, result, start, mid - 1);
            serializeHelper(root.right, result, mid + 1, end);
        }
    }
    ...
    private TreeNode deserialize(String[] nodes, int start, int end) {
        if (start < end) {
            int mid = start + (end - start)/2;
            if (nodes[mid] != null) {
                TreeNode nodeLeft = deserialize(nodes, start, mid - 1);
                TreeNode nodeRight = deserialize(nodes, mid + 1, end);
                TreeNode node = new TreeNode(Integer.parseInt(nodes[mid]));
                node.left = nodeLeft;
